...
def writemongodb(data, tissue, mongo_client):
    #Conexión a la coleccion
    gtex_db_tissue = mongo_client[tissue]
...
if __name__ == '__main__':
    ...
    for row in tqdm(contenido_db):          #Por cada linea en la DB (por cada gen)...
        for i in range(2, len(row)):        #Por cada columna en la DB (por cada muestra)
            try:
                d = {"expression": float(row[i]), "gene": row[1], "sample_id": header[i], "tissue_detail": ann[header[i]]["tissue_detail"]}
    ...
    gct_file.close()
    print("FINALIZADO!\n\tSe cargaron " + str(count) + " registros!")
    # Son aproximadamente 1.000.000.000 de documentos!